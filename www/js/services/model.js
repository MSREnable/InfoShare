...
mod.factory('model', [
    '$rootScope', '$q', '$firebaseObject', '$firebaseArray', 'auth',
        ...
        return {
            ...
            loadCachedPartner: function (uid) {
                ...
                if (!partner.permissions)
                    partner.permissions = $firebaseObject(permissions.child(uid).child(partner.level.$value + ''));
                if (!partner.blocks)
                    partner.blocks = $firebaseArray(blocks.child(uid).orderByKey().limitToLast(1));

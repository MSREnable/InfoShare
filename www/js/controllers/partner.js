...
mod.controller("partner", [
    '$scope', '$rootScope', '$stateParams', '$timeout', '$firebaseObject', '$firebaseArray', 'model', '$ionicSlideBoxDelegate',
    function ($scope, $rootScope, $stateParams, $timeout, $firebaseObject, $firebaseArray, model, $ionicSlideBoxDelegate) {
        ...
        $scope.conditionalClasses = function (filtered) {
            if (filtered) {
                if (($scope.aacuser.permissions.speech < 4 && $scope.aacuser.info.state < 3) || ($scope.aacuser.permissions.speech === 4 && $scope.aacuser.info.state < 2)) {
                    return 'block-card';
                }
            }

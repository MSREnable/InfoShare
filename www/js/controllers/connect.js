...
mod.filter('emailOrName', function () {
    return function (items, query) {
        ...
        angular.forEach(items, function (item) {
            if (item.$id !== window.localStorage.getItem('uid') && (item.first.indexOf(query) === 0 || item.last.indexOf(query) === 0 || uidToEmail(item.$id).indexOf(query) === 0)) {
                filtered.push(item);
            }
        });
...
mod.controller('connect', [
    '$scope', '$state', '$ionicPopup', 'model',
    function ($scope, $state, $ionicPopup, model) {
        ...
        $scope.isPartner = function (uid) {
            return _.find($scope.partners, function (p) {
                return p.uid === uid;
            });
        };

        $scope.isPending = function (uid) {
            return _.find($scope.pending, function (p) {
                return p.uid === uid;
            });
        };
        ...
        function showSuccess(name) {
            var alertPopup = $ionicPopup.alert({
                title: 'Request Sent!',
                template: 'Your request to connect with ' + name + ' has been sent. ' + name + ' will appear in your contacts on the home screen when they respond' + ' to your request.'
            });
        ...
        $scope.sendRequest = function (e) {
            ...
            if (!_.find($scope.partners, function (p) {
                return p.uid === e.$id;
            })) {
